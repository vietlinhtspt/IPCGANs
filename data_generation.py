...
if __name__ == '__main__':
    D=Detector("/home/linhnv/projects/IPCGANs-Pytorch/checkpoint/IPCGANS/2020-08-25_07-44-01/saved_parameters/gepoch_0_iter_12500.pth")
    # raw_img=Image.open("/media/2tb/data/detected_img/faces/1.jpg")
    # raw_img = Image.open("/home/linhnv/projects/IPCGANs-Pytorch/aged_img_1.jpg")
    # print(raw_img.size)
    # raw_img.save("raw_img.jpg")
    
    # for i in range(5):
    #     aged_img = D.demo(raw_img, target=i)
    #     aged_img.resize((112, 112), Image.ANTIALIAS)
    #     aged_img.save(f"aged_img_{i}.jpg")

    aged_folder = Path("/home/linhnv/projects/IPCGANs-Pytorch/aged_imgs")
    
    list_raw_imgs = glob.glob("/media/2tb/data/detected_img/faces/*")
    print(len(list_raw_imgs))

    for img_path in tqdm.tqdm(list_raw_imgs[:10000]):
        prefix = os.path.splitext(os.path.basename(img_path))[0]
        folder = aged_folder/prefix
        ...
        for i in range(5):
            aged_img = D.demo(raw_img, target=i)
            aged_img.resize((112, 112), Image.ANTIALIAS)
            aged_img.save(folder / f"aged_img_{i}.jpg")
