...
class IPCGANs:
    def __init__(self,lr=0.01,age_classifier_path=None,gan_loss_weight=75,feature_loss_weight=0.5e-4,age_loss_weight=30):
        ...
        if age_classifier_path is not None:
            self.age_classifier=AgeAlexNet(pretrainded=False,modelpath=age_classifier_path).cuda()
        else:
    ...
    def load_generator_state_dict_custom(self, model_path):
        self.generator.load_state_dict(torch.load("g" + model_path))
        self.discriminator.load_state_dict(torch.load("d" + model_path))
